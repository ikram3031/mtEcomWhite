# Decantre Frontend API Integration & State Flow Documentation

This document provides a comprehensive and exhaustive guide to how API communication, authentication flows, data mapping, and state management are implemented in the Decantre frontend.
...
export async function fetchProducts(opts = {}) {
  ...
  try {
    ...
    mapped._totalRows =
      json.meta?.total_products ?? json.totalRows ?? list.length;
    return mapped;
  } catch (err) {
...
export async function fetchProductDetails(slugOrId) {
  ...
  try {
    ...
    return targetData && typeof targetData === "object"
      ? mapRemoteProduct(targetData)
      : null;
  } catch (err) {
...
- **Purpose**: Checks if user exists and determines if login or registration is required.
- **Used In**: `AuthModal.jsx` (Determines the next auth screen flow).
...
- **Purpose**: Log in user with credentials.
...
- **Purpose**: Registration step. Initiates OTP verification.
...
- **Purpose**: Validates the 6-digit OTP code to authorize registration or changes.
...
These routes use the authorized fetch wrapper `authFetch` and require active access tokens.
...
- **Purpose**: Modifies existing account details, addresses, and user profiles.
...
## 5. Product Schema Mapping & Helpers
...
export const mapRemoteProduct = (product = {}) => {
  ...
  return {
    ...
    brand: resolveBrandName(product.brand),
    basePrice:
...
## 6. Store Integration & Global State Management
