...
export const getAnalyticsDataForRange = (range = '30days', brandName = null, storeStats = null) => {
  ...
  const hasStoreStats = storeStats && typeof storeStats.netSales === 'number' && storeStats.netSales > 0;
  ...
  if (range === 'today') {
    for (let h = 0; h <= 23; h++) {
      ...
      const hourlySessions = Math.max(1, Math.round(hourActivity * (baseSessions / 12)));
      const hourlyUsers = Math.max(1, Math.round(hourlySessions * 0.78));
      const hourlyViews = Math.max(1, Math.round(hourlySessions * 2.8));
      const hourlyPurchases = baseTransactions > 0 && h >= 10 && h <= 22 ? 1 : 0;
    ...
    for (let i = daysCount - 1; i >= 0; i--) {
      ...
      const dailySessions = Math.max(1, Math.round((baseSessions / daysCount) * dayFactor));
      const dailyUsers = Math.max(1, Math.round(dailySessions * 0.76));
      const dailyViews = Math.max(1, Math.round(dailySessions * 3.1));
      const dailyPurchases = baseTransactions > 0 ? Math.max(0, Math.round((baseTransactions / daysCount) * dayFactor)) : 0;
  ...
  const funnelStages = [
    ...
    },
    {
      ...
      eventName: 'purchase',
      count: baseTransactions || Math.round(baseSessions * 0.027),
      dropoffRate: '43.2%',
  ...
  const kpis = {
    realtimeActive: industry.realtimeActive || 7,
    totalSessions: baseSessions,
    totalUsers: baseUsers,
    totalPageviews: basePageviews,
    viewsPerSession: baseSessions > 0 ? (basePageviews / baseSessions).toFixed(2) : '3.02',
...
export const getRealtimeMinuteActivity = () => {
  ...
  const maxRange = Math.max(2, industry.realtimeActive || 7);
  const minutes = [];
  const now = new Date();
  for (let i = 29; i >= 0; i--) {
    const t = new Date(now.getTime() - i * 60000);
    const label = `${t.getHours().toString().padStart(2, '0')}:${t.getMinutes().toString().padStart(2, '0')}`;
    const value = Math.max(1, Math.round(((Math.sin((i / 30) * Math.PI) + 0.3) * maxRange)));
    minutes.push({
      time: label,
      activeUsers: value,
    });
